...
async function sendInviteEmail({ to, inviterName, role, acceptUrl, expiresInDays }) {
  const inviter = inviterName || 'A Polaris Point admin';
  const days = expiresInDays || 7;
  const roleLabel = role === 'senior' ? 'Senior Admin' : 'Team Member';
  const html = emailShell({
    headline: `You're invited to the Polaris Point admin`,
    bodyHtml: `
      <p>${escapeHtml(inviter)} invited you to join the Polaris Point admin as a <strong>${escapeHtml(roleLabel)}</strong>.</p>
      <p>Click below to set your name and password and activate your account. The link works once and expires in ${days} days.</p>`,
    ctaUrl: acceptUrl,
    ctaText: 'Accept invitation',
    footnoteHtml: `If you weren't expecting this, you can ignore it — no account exists until you accept. Questions? Reply to this email.`
  });
  const text = `You're invited to the Polaris Point admin.

${inviter} invited you to join as a ${roleLabel}.

Accept your invitation (expires in ${days} days):
${acceptUrl}

If you weren't expecting this, ignore this email.`;
  return sendEmail({ to, subject: 'Your Polaris Point admin invitation', html, text, replyTo: 'peter@polarispoint.io' });
}
