...
const DEFAULT_SETTINGS = {
  ...
  // NOTE: `onboarded` is intentionally NOT here. It's set explicitly to false
  // in /api/beacons/auth/create.js for new tenants, and explicitly to true in
  // ensurePeteTenant() for Pete's legacy account. This way the wizard auto-
  // triggers only for accounts that were created through the new auth flow.
};
...
async function ensureTenantsTable(sql) {
  ...
  // Idempotent migration: existing rows on legacy tier names get renamed.
  // Safe to re-run — UPDATEs only fire when a row still has the old name.
  await sql`UPDATE beacons_tenants SET tier = 'spark'      WHERE tier = 'basic'`;
  await sql`UPDATE beacons_tenants SET tier = 'beam'       WHERE tier = 'premium'`;
  await sql`UPDATE beacons_tenants SET tier = 'lighthouse' WHERE tier = 'pro'`;
...
async function ensurePeteTenant(sql) {
  ...
  // Pete's tenant is marked onboarded:true so the wizard doesn't fire for him —
  // his existing directions item already serves as priming.
  const peteSettings = { ...DEFAULT_SETTINGS, onboarded: true };
  await sql`
    ...
    VALUES ('pete', ${placeholderEmail}, ${placeholderHash}, 'Pete', 'pete', 100,
            ${JSON.stringify(peteSettings)}::jsonb, TRUE)
