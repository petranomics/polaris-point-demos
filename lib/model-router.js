...
//   claude-sonnet-4-6   — mid (research, multi-step, paid-tier chat)
// Opus is intentionally disabled across all tiers — every call we billed
// for in May was one-shot and never re-read its cache, so we were paying
// the cache-write surcharge at Opus rates for no benefit. Re-enable per
// tier via TIER_CAPS.allow_opus if a specific workload genuinely needs it.
//
// Models (Ollama):
...
// Opus is off across the board (see header comment). Pete is also kept on
// Haiku only — personal-use chat doesn't need Sonnet, and routing Sonnet on
// every "deep" turn for one user was still costing more than the value.
//
// Tier names match lib/tier-limits.js (lighthouse brand):
//   spark / beam / lighthouse — paid tiers
//   free                       — pre-paid trial
//   pete                       — internal admin
// Legacy keys (basic/premium/pro) auto-map via LEGACY_TIER_ALIASES below
// so existing DB rows keep working until they're migrated.
const TIER_CAPS = {
  pete:       { allow_opus: false, allow_sonnet: false, allow_haiku: true,  allow_ollama: true },
  lighthouse: { allow_opus: false, allow_sonnet: true,  allow_haiku: true,  allow_ollama: true },
  beam:       { allow_opus: false, allow_sonnet: true,  allow_haiku: true,  allow_ollama: true },
  spark:      { allow_opus: false, allow_sonnet: false, allow_haiku: true,  allow_ollama: true },
  free:       { allow_opus: false, allow_sonnet: false, allow_haiku: false, allow_ollama: true }
};
const LEGACY_TIER_ALIASES = { basic: 'spark', premium: 'beam', pro: 'lighthouse' };

function tierCaps(tenant) {
  const raw = tenant?.tier;
  const t = (raw && LEGACY_TIER_ALIASES[raw]) || raw;
  return TIER_CAPS[t] || TIER_CAPS.spark;
}
