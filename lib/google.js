...
const GOOGLE_SCOPES = [
  ...
  // Full Drive read+write — needed so Beacon can edit existing files (e.g.,
  // append rows to a Sheet you already created), not just files it created.
...
// Accept BOTH legacy passcode hash AND JWT in x-beacons-auth header.
// Legacy: 64-char sha256 hex of passcode → compared to BEACONS_PASSCODE_HASH.
// New: JWT signed with BEACONS_JWT_SECRET → verified inline.
// Either passes → authenticated. Stays sync so call sites don't have to await.
function checkBeaconsAuth(req) {
  const got = (req.headers['x-beacons-auth'] || '').toString();
  if (!got) return false;

  // JWT path: tokens have dots and are >80 chars. Verify against JWT secret.
  if (got.includes('.') && got.length > 80 && process.env.BEACONS_JWT_SECRET) {
    try {
      const jwt = require('jsonwebtoken');
      const claims = jwt.verify(got, process.env.BEACONS_JWT_SECRET);
      if (claims && claims.sub) return true;
    } catch (_) { /* fall through to legacy */ }
  }

  // Legacy passcode hash path.
...
function checkConfig() {
  // Reports which env vars are missing for the Google integration to work.
  // Auth requires EITHER BEACONS_JWT_SECRET (new) OR BEACONS_PASSCODE_HASH (legacy).
  const missing = [];
  if (!process.env.DATABASE_URL) missing.push('DATABASE_URL');
  if (!process.env.BEACONS_JWT_SECRET && !process.env.BEACONS_PASSCODE_HASH) {
    missing.push('BEACONS_JWT_SECRET');
  }
